      ...
      for (int x = 0; x < width; x ++) {
        for (int y = 0; y < height; y ++) {
          ...
          if (map[x,y] == 0 || map[x,y] == 2) {
            GameObject g = GroundPrefab;
            g = (GameObject)Instantiate(GroundPrefab, transform.position = new Vector3(posX, 0, posY), Quaternion.identity);
            g.GetComponent<WallData>().mapX = x;
            g.GetComponent<WallData>().mapY = y;
            g.GetComponent<WallData>().i = map[x,y];
          }
          else {
            GameObject w = WaterPrefab;
            w = (GameObject)Instantiate(WaterPrefab, transform.position = new Vector3(posX, 0, posY), Quaternion.identity);
            w.GetComponent<WallData>().mapX = x;
            w.GetComponent<WallData>().mapY = y;
            w.GetComponent<WallData>().i = map[x,y];
            w.name = "Water";
          }
        }
      }
